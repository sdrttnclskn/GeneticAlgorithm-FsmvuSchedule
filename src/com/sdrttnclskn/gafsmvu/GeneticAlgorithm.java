...
public class GeneticAlgorithm {
	...
	Population crossoverPopulation(Population population) {
		...
		IntStream.range(0, MainSchedule.NUMB_OF_ELITE_SCHEDULES).forEach(x -> crossoverPopulation.getSchedules().set(x,
    ...
    Population mutatePopulation(Population population) {
    	...
    	IntStream.range(0, MainSchedule.NUMB_OF_ELITE_SCHEDULES).forEach(x -> schedules.set(x, population.getSchedules().get(x)));
    ...
    Schedule mutateSchedule(Schedule mutateSchedule) {
    	...
    	IntStream.range(0, mutateSchedule.getClasses().size()).forEach(x -> {
    		if (MainSchedule.MUTATION_RATE > Math.random()) mutateSchedule.getClasses().set(x, schedule.getClasses().get(x));
    	});
